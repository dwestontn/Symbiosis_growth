...
#There are 277 metabolites measured in the cross feeding. to figure out which of the 277 were not eluted
#in any of the samples, use this code:
#trash<-met.dat %>% filter(treat_cond =="Ph1-super-Sfallax") %>% group_by(JGI_compound_id) %>% tally(peak_ht==0)
#trash%>%filter(n==4)
#view(trash)
...
Cyano elluted metabolites when fed moss super

```{r cars}

crossfeed_PelCyano_MossSuper %>% filter(met_direction == "eluted_by cyano" & p.value <=0.05) %>% group_by(JGI_compound_id) %>% mutate(per_change_elluted = ((mean_Pelcyano_SupMoss - mean_super_moss)/mean_super_moss) * 100) %>% arrange(p.value, -per_change_elluted) %>% select(JGI_compound_id, label, p.value, per_change_elluted, mean_Pelcyano_SupMoss, mean_super_moss) %>% kable(., caption = "Cyano elluted metabolites when fed moss super")
...
Moss elluted metabolites when fed cyano super 

```{r, echo=FALSE}

crossfeed_PelCyano_MossSuper %>% filter(met_direction == "eluted_by cyano" & p.value <=0.05) %>% group_by(JGI_compound_id) %>% mutate(per_change_elluted = ((mean_Pelcyano_SupMoss - mean_super_moss)/mean_super_moss) * 100) %>% arrange(p.value, -per_change_elluted) %>% select(JGI_compound_id, label, p.value, per_change_elluted, mean_Pelcyano_SupMoss, mean_super_moss) %>% kable(., caption = "Cyano elluted metabolites when fed moss super")

#to save table as csv
#crossfeed_PelCyano_MossSuper <- crossfeed_PelCyano_MossSuper %>% filter(met_direction == "eluted_by cyano" & p.value <=0.05) %>% group_by(JGI_compound_id) %>% mutate(per_change_elluted = ((mean_Pelcyano_SupMoss - mean_super_moss)/mean_super_moss) * 100) %>% arrange(p.value, -per_change_elluted) %>% select(JGI_compound_id, label, p.value, per_change_elluted, mean_Pelcyano_SupMoss, mean_super_moss)

#write.csv(crossfeed_PelCyano_MossSuper, "crossfeed_PelCyano_MossSuper.csv")
