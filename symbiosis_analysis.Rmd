...
## Growth 2017 Statistical Analysis and Results
The experiment was a 4 x 3 factorial design with Condition and pH as factors. Condition refers to the spectrum of interactions between moss and cyanobacteria with moss grown either alone (moss_alone), with cyanobacteria (Cyanobacteria_moss_symbiosis), separated with a filter (Cyanobacteria_moss_seperate), and lastly, with cyanobacteria grown alone (cyano_alone). Moss and cyanobacteria in each treatment were each grown in pH 3.5, pH 5.5, or pH 8.5 (n = 6 for each Condition and pH combination). Growth data based on the difference between final and initial weight of moss were rank transformed prior to performing a two-way ANOVA using Condition and pH as independent variables and an alpha = 0.05 to denote significance. Post-hoc tests using Tukey's HSD was performed to determine differences between Condition and pH.

The driving determinant for growth was Condition (p < 0.01), with moss growth with cyanobacteria (cyanobacteria_moss_symbiosis) producing the highest biomass (74.77 g +/- 30.69 at pH 8.5). Regardless of the pH, moss grown with cyanobacteria consistently grew more than moss that was separated from cyanobacteria using a filter (54 g +/- 51.43 at pH 8.5; p < 0.05) and moss grown alone (24.24 g +/- 15.35 at pH 8.5; p < 0.05). There was no significant difference between moss grown alone and moss separated from cyanobacteria ( p = 0.81).

As expected, cyanobacterial mass was significantly lower (29.80 g +/- at pH 8.5; p < 0.05) and differed greatly when compared to cyano growth with moss either together (p < 0.05) and separated with a filter (p < 0.05). However, cyanobacteria grown alone did not differ to moss grown alone (24.25 +/- at pH 8.5; p = 0.10). Additionally, moss grown alone was similar to moss separated from cyanobacteria (p = 0.82), suggesting that direct contact between cyanobacteria and moss is necessary to increasing growth under any pH.

Although pH did not significantly alter growth within each condition, there was a trend of increasing mean biomass with increasing biomass. Without exception, growth in each condition was higher at pH 8.5, particularly in moss grown directly with cyanobacteria and moss separated from cyanobacteria with a filter. These two conditions consistently yielded the highest biomass, with cyanobacteria grown alone typically having the lowest (1.38 g +/- 9.17 at pH 3.5 and 5.64 g +/- 16.41 at pH 5.5).
...
library(tidyverse)
dat2 <- read.csv("fallax_cyano_symbiosis_allyssa_colleen_2017.csv")
...
tri.2017 <- read.csv("fallax_cyano_symbiosis_allyssa_colleen_2017.csv")
View(tri.2017)
...
# Both condition and pH are significant in controlling growth, with condition being the most significant variable.the interaction term is not significant suggesting that pH and condition influence growth independently of each other. thus we do post-hoc tests just on the main effects

TukeyHSD(tri.2017.aov, which = c("condition", "pH"))

# compact letter display for each factor
cld(lsmeans(tri.2017.aov, ~condition))
cld(lsmeans(tri.2017.aov, ~pH))
...
1 - (1.508940/2) # 0.24553 is the power
...
```{r Testing ANOVA - Cube Transform}

# include cube root just for comparison
tri.2017.cube <- cbind(tri.2017, weight.cubed)
tri.2017.cube$pH <- as.factor(tri.2017.cube$pH)
str(tri.2017.cube)

tri.2017.exp.aov <- aov(weight.cubed ~ condition + pH + condition:pH, data = tri.2017.cube)
summary(tri.2017.exp.aov) # no change in outcome of ANOVA

## Shapiro-Wilk test - normality check
tri2017.transresiduals2 <- residuals(object = tri.2017.exp.aov)
shapiro.test(x = tri2017.transresiduals2) # p value is significant. normality is violated.
...
library(car)
leveneTest(weight.cubed ~ condition*pH, data = tri.2017.cube) # not homogenous.
```
...
weight.rank <- rank(tri.2017$delta_weight)
tri.2017 <- cbind(tri.2017, weight.rank)
tri.2017$pH <- as.factor(tri.2017$pH)

tri2017.rank.aov <- aov(weight.rank ~ condition + pH + condition:pH, data = tri.2017)
Anova(tri2017.rank.aov, type = 3)
TukeyHSD(tri2017.rank.aov, which = "condition")
...
tri.2018 <- read.csv("fallax_cyano_symbiosis_colleen_10_01_2018.csv") %>% mutate(delta_weight = delta_weight_g *1000)
View(tri.2018)
...
## metablite change when moss pellet is crossfed with fungus supernantant
...
k2<-met.dat %>% select(treat_cond, JGI_compound_id, peak_ht) %>% filter(treat_cond %in% c("Ph2-super-Sfallax-TacroSupp", "Ph1-super-Tacro"))

#to account for the dillution from phase 1 to phase 2
k2<-k2 %>% mutate(peak_ht = ifelse(treat_cond == "Ph1-super-Tacro", peak_ht/2, peak_ht * 1))
k2$JGI_compound_id <- as.factor(k2$JGI_compound_id)
k2$treat_cond <- as.factor(k2$treat_cond)

#Kruskal Wallis test
l2<- k2 %>% group_by(JGI_compound_id) %>% do(tidy(kruskal.test(x= .$peak_ht, g = .$treat_cond))) %>% select(JGI_compound_id, statistic, p.value)

#datatable creation
m2 <-k2 %>% filter(treat_cond == "Ph2-super-Sfallax-TacroSupp") %>% group_by(JGI_compound_id) %>% mutate(mean_Pelmoss_SupFung = mean(peak_ht)) %>% select(JGI_compound_id, mean_Pelmoss_SupFung) %>% unique()

m22 <- k2 %>% filter(treat_cond == "Ph1-super-Tacro") %>% group_by(JGI_compound_id) %>% mutate(mean_super_fung = mean(peak_ht)) %>% select(JGI_compound_id, mean_super_fung) %>% unique()

m32 <- left_join(m2,m22, by = "JGI_compound_id") %>% mutate(met_direction = ifelse(mean_Pelmoss_SupFung > mean_super_fung, "eluted_by moss",  "uptake_up by moss"))

crossfeed_PelMoss_FungSuper<- met.dat %>% select(JGI_compound_id, compound_atlas_ID, label, Identified_metabolite) %>% unique()

crossfeed_PelMoss_FungSuper$JGI_compound_id <-as.factor(crossfeed_PelMoss_FungSuper$JGI_compound_id)
crossfeed_PelMoss_FungSuper$compound_atlas_ID <-as.factor(crossfeed_PelMoss_FungSuper$compound_atlas_ID)
crossfeed_PelMoss_FungSuper$Identified_metabolite <-as.factor(crossfeed_PelMoss_FungSuper$Identified_metabolite)

crossfeed_PelMoss_FungSuper<-as.data.frame(crossfeed_PelMoss_FungSuper)
m32<-as.data.frame(m32)
crossfeed_PelMoss_FungSuper <- left_join(m32, crossfeed_PelMoss_FungSuper, by = "JGI_compound_id")

crossfeed_PelMoss_FungSuper<-left_join(l2, crossfeed_PelMoss_FungSuper, by = "JGI_compound_id")
crossfeed_PelMoss_FungSuper<-crossfeed_PelMoss_FungSuper %>% arrange(p.value)

#output
kable(crossfeed_PelMoss_FungSuper[1:8, c(-1, -2, -4,-5)], caption = "Header- crossfed moss pellet with fungus supernatant")


num_sig_elluted<-nrow(crossfeed_PelMoss_FungSuper %>% filter(p.value <= 0.05 & met_direction == "eluted_by moss"))
num_sig_uptake<-nrow(crossfeed_PelMoss_FungSuper %>% filter(p.value <= 0.05 & met_direction == "uptake_up by moss"))

print(paste("# elluted fungal metabolites (p.value <= 0.05) is:", num_sig_elluted))
print(paste("# taken fungal metabolites (p.value <= 0.05) is:", num_sig_uptake))

#write_excel_csv(crossfeed_PelMoss_FungSuper, "crossfeed_PelMoss_FungSuper_halfstrength.csv")

```


## metablite change when cyano pellet is crossfed with fungus supernantant
...
########something is wrong with below code

```{r, echo=FALSE}

k23<-met.dat %>% select(treat_cond, JGI_compound_id, peak_ht) %>% filter(treat_cond %in% c("Ph2-super-Nmusc-TacroSupp", "Ph1-super-Tacro"))

#to account for the dillution from phase 1 to phase 2
k23<-k23 %>% mutate(peak_ht = ifelse(treat_cond == "Ph1-super-Tacro", peak_ht/2, peak_ht * 1))
k23$JGI_compound_id <- as.factor(k23$JGI_compound_id)
k23$treat_cond <- as.factor(k23$treat_cond)

#Kruskal Wallis test
l23<- k23 %>% group_by(JGI_compound_id) %>% do(tidy(kruskal.test(x= .$peak_ht, g = .$treat_cond))) %>% select(JGI_compound_id, statistic, p.value)

#datatable creation
m23 <-k23 %>% filter(treat_cond == "Ph2-super-Nmusc-TacroSupp") %>% group_by(JGI_compound_id) %>% mutate(mean_Pelcyano_SupFung = mean(peak_ht)) %>% select(JGI_compound_id, mean_Pelcyano_SupFung) %>% unique()

m223 <- k23 %>% filter(treat_cond == "Ph1-super-Tacro") %>% group_by(JGI_compound_id) %>% mutate(mean_super_fung = mean(peak_ht)) %>% select(JGI_compound_id, mean_super_fung) %>% unique()

m323 <- left_join(m23,m223, by = "JGI_compound_id") %>% mutate(met_direction = ifelse(mean_Pelcyano_SupFung > mean_super_fung, "eluted_by cyano",  "uptake_up by cyano"))

crossfeed_PelCyano_FungSuper<- met.dat %>% select(JGI_compound_id, compound_atlas_ID, label, Identified_metabolite) %>% unique()

crossfeed_PelCyano_FungSuper$JGI_compound_id <-as.factor(crossfeed_PelCyano_FungSuper$JGI_compound_id)
crossfeed_PelCyano_FungSuper$compound_atlas_ID <-as.factor(crossfeed_PelCyano_FungSuper$compound_atlas_ID)
crossfeed_PelCyano_FungSuper$Identified_metabolite <-as.factor(crossfeed_PelCyano_FungSuper$Identified_metabolite)

crossfeed_PelCyano_FungSuper<-as.data.frame(crossfeed_PelCyano_FungSuper)
m323<-as.data.frame(m323)
crossfeed_PelCyano_FungSuper <- left_join(m323, crossfeed_PelCyano_FungSuper, by = "JGI_compound_id")

crossfeed_PelCyano_FungSuper<-left_join(l23, crossfeed_PelCyano_FungSuper, by = "JGI_compound_id")
crossfeed_PelCyano_FungSuper<-crossfeed_PelCyano_FungSuper %>% arrange(p.value)

#output
kable(crossfeed_PelCyano_FungSuper[1:8, c(-1, -2, -4,-5)], caption = "Header- crossfed moss pellet with fungus supernatant")


num_sig_elluted<-nrow(crossfeed_PelCyano_FungSuper %>% filter(p.value <= 0.05 & met_direction == "eluted_by cyano"))
num_sig_uptake<-nrow(crossfeed_PelCyano_FungSuper %>% filter(p.value <= 0.05 & met_direction == "uptake_up by cyano"))

print(paste("# elluted cyano metabolites (p.value <= 0.05) is:", num_sig_elluted))
print(paste("# taken cyano metabolites (p.value <= 0.05) is:", num_sig_uptake))

#write_excel_csv(crossfeed_PelCyano_FungSuper, "crossfeed_PelCyano_FungSuper_halfstrength.csv")
