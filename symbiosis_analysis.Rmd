...
Supernatant changes from 1) suernatant from moss pellet when incubated with fungus supernatant (Ph2-super-Nmusc-TacroSupp) compared to 2) fungus pellet only supernantant (Ph1-super-Tacro).
...
num_sig_elluted<-nrow(crossfeed_PelCyano_FungSuper %>% filter(p.value <= 0.05 & met_direction == "eluted_by moss"))
num_sig_uptake<-nrow(crossfeed_PelCyano_FungSuper %>% filter(p.value <= 0.05 & met_direction == "uptake_up by moss"))
